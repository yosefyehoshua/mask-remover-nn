...
from stylegan_layers import G_mapping,G_synthesis, get_noise_params, delete_noise_param
...
def main():
    ...
    ('g_synthesis', G_synthesis(resolution=args.resolution))    
     ...
     for i in range(args.iteration):
          ...
          if i%100==0:
              ...
              save_image(synth_img.clamp(0, 1),
                         "save_image/crossover/{}.png".format(i))
     ...
     #print(len(noise_arr))
     #print(noise_arr[0].shape)
     # noise optimization
     #dn = torch.zeros((1, 18, 512),requires_grad=True, device=device)
     ...
     for i in range(args.iteration):
          ...
          if i%100==0:
               ...
               save_image(synth_img.clamp(0,1),"save_image_noise/crossover/{}.png".format(i))
               np.save("noise_N/crossover.npy",dlatent.detach().cpu().numpy())
          delete_noise_param()
